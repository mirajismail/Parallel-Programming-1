...
void one_time_step(std::vector<int>& x, std::vector<int>& v, const Parameters& params)
{
    ...
    std::vector<float> randvals(N);
    for (int i = 0; i < N; i++)
        randvals[i] = random_next_float();


    #pragma omp parallel for default(none) shared(x, v, params, randvals, N, x0)
    for (int i = 0; i < N; i++) {
        ...
        if (randvals[i] < params.p && v[i] > 0) {
            v[i] --;
        }
       
    }
    
    #pragma omp parallel for default(none) shared(x, v, N)
    for (int i = 0; i < N; i++) {
